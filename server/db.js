...
// ---------- Postgres (persistent - Render + Aiven) ----------
if (usePostgres) {
  console.log('[DB] Using Postgres (DATABASE_URL set)');
  const { Pool } = require('pg');
  // Aiven uses self-signed cert - must disable verification
  // Strip sslmode from URL to prevent pg's libpq handling from overriding our ssl config
  let connStr = process.env.DATABASE_URL.replace(/[?&]sslmode=[^&]+/g, '').replace(/\?$/, '');
  pool = new Pool({
  ...
  function toPg(sql) {
    ...
    // INSERT OR REPLACE -> Postgres upsert (must replace OR REPLACE first)
    if (/INSERT OR REPLACE INTO content/i.test(sql)) {
      pgSql = pgSql.replace(/INSERT OR REPLACE INTO content/i, 'INSERT INTO content');
      pgSql = pgSql.replace(/INSERT INTO content\s*\(key,\s*value\)\s*VALUES\s*\(\$1,\s*\$2\)/i,
        'INSERT INTO content (key, value) VALUES ($1, $2) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value');
    }
    // Add RETURNING id for inserts where we need lastInsertRowid
    const lower = sql.trim().toLowerCase();
    if (lower.startsWith('insert into') && !pgSql.toLowerCase().includes('returning')) {
      if (lower.includes('into media') || lower.includes('into leads') || lower.includes('into events') || lower.includes('into admin_users')) {
        pgSql += ' RETURNING id';
      }
    }
    return pgSql;
    ...
    prepare(sql) {
      ...
      return {
        get: async (...params) => {
          ...
          // normalize count(*) -> c (pg returns string)
          if (row && row.c !== undefined) row.c = Number(row.c);
          if (row && row.count !== undefined && row.c === undefined) row.c = Number(row.count);
          return row;
        },
        all: async (...params) => {
          const res = await pool.query(pgSql, params);
        ...
        run: async (...params) => {
          ...
          return { lastInsertRowid: id, changes: res.rowCount, rowCount: res.rowCount };
        }
      };
    },
    exec: async (sql) => {
...
async function initDb() {
  if (usePostgres) {
    ...
    const existing = await db.prepare('SELECT COUNT(*) as c FROM content').get();
    ...
    if (Number(adminCount.c) === 0) {
      ...
      console.log('[DB] Default admin created on Postgres: admin / admin123');
    }
    const mediaCount = await db.prepare('SELECT COUNT(*) as c FROM media').get();
    if (Number(mediaCount.c) === 0) {
      ...
      for (const r of portfolio) await db.prepare('INSERT INTO media (category, url, caption, sort_order) VALUES (?, ?, ?, ?)').run(...r);
      console.log('[DB] Seeded Postgres media');
    }
    return;
  }
...
module.exports = { db, initDb, useSQLite, usePostgres, pool };
